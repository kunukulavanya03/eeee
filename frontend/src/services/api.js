...
export const api_resources_{id} = async (id, data) => {
  const response = await api.put(`/api/resources/{id}/${id}`, data);
  return response.data;
};
export const api_resources_{id} = async (id) => {
  const response = await api.delete(`/api/resources/{id}/${id}`);
  return response.data;
};
